...
public class CadastroProdutosController extends HttpServlet {
    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        String dataVencimentoParam = request.getParameter("dataVencimento");
        if (dataVencimentoParam != null && !dataVencimentoParam.isBlank()) {
            LocalDate dataVencimento = LocalDate.parse(dataVencimentoParam);
            if (dataVencimento.isBefore(LocalDate.now())) {
                response.sendRedirect("pages/cadastroProduto.html?erro=vencido");
                return;
            }
        }
        ...
        if (dataFabricacaoParam != null && !dataFabricacaoParam.isBlank()) {
            ...
            if (dataFabricacao.isAfter(LocalDate.now())) {
                response.sendRedirect("pages/cadastroProduto.html?erro=fabricacaoFutura");
                return;
            }
        }
